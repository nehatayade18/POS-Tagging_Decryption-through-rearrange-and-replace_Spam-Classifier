...
words, tags, cntTags, initProbs, tranProbTable, PSi = solver.train(train_data)
...
for (s, gt) in test_data:
    emissProbTable = solver.emissProb(s, words, tags, cntTags)
    #emissProbTable1 = solver.emissProb1(s, words, tags, cntTags)
    ...
    for (algo, label) in zip(Algorithms, Algorithm_labels):
        outputs[label], outPost = solver.solve( algo, s, words, tags, cntTags, initProbs, tranProbTable, PSi, emissProbTable)
        prob.append(outPost)
    # calculate posteriors for each output under each model
    posteriors = { o: { a: solver.posterior( a, s, outputs[o], prob[0], prob[1], prob[2]) for a in Algorithms } for o in outputs }
